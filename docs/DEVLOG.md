...
## Phase 8: 仕上げ(2026-07-08)

### やったこと

- **README を英日2本作成**(`README.md` 英語=主、`README.ja.md` 日本語)。GitHub トップに出るのは英語版。
  構成図は GitHub がネイティブ描画する Mermaid、結果表は RESULTS.md の要約、CI バッジ、セットアップ手順、
  進捗(Phase 7 の実機生成が未確認である旨を正直に記載)、「読みどころ」の設計判断リストを含む。
- **UI 磨き込み**(破壊的変更なし):
  - 検索画面に検索前の初期ガイド状態を追加(「意味で探す」+ 例)。
  - 類似度スコアを関連度バッジ(Capsule + tint)に変更(従来は開発用の素の数値表示)。
  - ノート一覧に更新日時(相対表示)を追加。
- **デモ動画台本**(`docs/DEMO_SCRIPT.md`): 90秒〜2分のカット割り。日本語ナレーション+英語字幕案。
...
- **README の言語**: 出願先が英語圏 CS/AI のため、GitHub トップに出る `README.md` を英語にし、日本語は副
  (`README.ja.md`)。CLAUDE.md の「英語圏なら英語版も」を、主副を逆にする形で満たした。
- **構成図の形式**: SVG(RESULTS の図と統一)か Mermaid か。アーキテクチャ図は差分が読めて保守しやすい
  Mermaid を選択(GitHub がネイティブ描画)。定量結果の図は引き続き SVG。
- **Phase 8 の完了扱い**: README・UI・台本・配布ドキュメントという成果物は用意完了だが、デモ動画の**録画**と
  TestFlight の**実配布**は開発者本人の作業(有償アカウント・実機)。コードで閉じない項目が残るため、
  PLAN の Phase 8 はまだチェックしない。
